...
public class Navi2 implements Runnable {
   ...
   //        "왼쪽","약 40m 이동","오른쪽","경희대학교 국제캠퍼스 공과대학관까지 약 94m 이동","경희대학교 국제 캠퍼스 공과대학관"};
    ...
    public static List<Marker> pathMarkerList2;
    public void run() {
            ...
            while (true) {
                ...
                if (isStart) {
                    ...
                    Log.d("Navi2 start", "" + isStart);

                    path = new double[pathMarkerList2.size()];
                    pathName = new String[pathMarkerList2.size()];


                    for (int i = 0; i < pathMarkerList2.size(); i++) {
                        path[i] = pathMarkerList2.get(i).getDistance();
                        pathName[i] = pathMarkerList2.get(i).getDescription();

                        Log.i("path", String.valueOf(path[i]));
                        Log.i("Pathname", String.valueOf(pathName[i]));
                    ...
                    while (temp < path.length) {
                        if (path[pathIndex] == 0) {
                            ...
                            Log.v("Navi2 arrive", " path :" + curpath + "도착");
                            ...
                            Log.v("Navi move", tempDistance + " path :" + curpath);
